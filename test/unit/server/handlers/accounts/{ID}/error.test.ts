...
describe('accounts error handler', (): void => {
  describe('PUT /accounts/{ID}/error', (): void => {
    beforeAll((): void => {
    ...
    it('handles a successful request', async (): Promise<void> => {
      mockForwardAccountsRequestError.mockResolvedValueOnce()

      // Act
      const response = await AccountsIdHandler.put(null, request, mockResponseToolkit)

      // Assert
      expect(response.statusCode).toBe(200)
      expect(mockForwardAccountsRequestError).toHaveBeenCalledTimes(1)
      expect(mockForwardAccountsRequestError).toHaveBeenCalledWith(...expected)
    })

    it('handles errors asynchronously', async () => {
      // Arrange
      mockForwardAccountsRequestError.mockRejectedValueOnce(new Error('Test Error'))
      // Act
      const response = await AccountsIdHandler.put(null, request, mockResponseToolkit)
